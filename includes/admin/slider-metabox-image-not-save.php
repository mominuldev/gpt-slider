...
//	}
	...
	if($custom_repeater_values) {
		...
		echo '<td>';
		echo '<div class="custom-media-field">';
		echo '<div class="custom-media-preview"></div>';
...
//	if (!isset($_POST['title']) || !isset($_POST['description']) || !isset($_POST['button_label']) || !isset($_POST['button_url'])) {
//		delete_post_meta($post_id, '_custom_repeater_values');
//		return;
//	}
	...
	for ($i = 0; $i < count($titles); $i++) {
		...
		if (!empty($title) || !empty($description) || !empty($button_url || !empty($image))) {
			$items[] = array(
				'title' => $title,
